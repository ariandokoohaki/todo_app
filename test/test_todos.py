...
def test_create_todo(client, test_db: Session):
    ...
    )  # Ensure user_id is string if JWT expects str
    todo_data = TodoCreate(
        title="test_todo_creation", description="test_description_creation"
    ).model_dump()  # UPDATED
    response = client.post(
        "/todos/", json=todo_data, headers={"Authorization": f"Bearer {token}"}
    )
...
def test_create_todo_unauthorised(
    client, test_db: Session
):  # test_db might not be needed if no setup
    todo_data = TodoCreate(
        title="test_todo_unauth", description="test_description_unauth"
    ).model_dump()  # UPDATED
...
def test_get_todos(client, test_db: Session):
    ...
    todo_to_create_data = TodoCreate(
        title="test_todo_for_get", description="test_desc_for_get"
    ).model_dump()  # UPDATED
    create_response = client.post(
        "/todos/",
        json=todo_to_create_data,
    ...
    found_todo = next(
        (item for item in response_json if item["title"] == "test_todo_for_get"), None
    )
