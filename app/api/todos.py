...
from app.models.todo import Todo  # Your SQLAlchemy Todo model
...
@router.post("/", response_model=TodoRead, status_code=status.HTTP_201_CREATED, summary="Create New Todo")
async def create_new_todo_api(  # Renamed to avoid conflict if create_todo_service was imported as 'create_todo'
...
async def read_own_todos_api(  # Renamed
    ...
    todos = await get_todos_by_owner(db,
                                     owner_id=current_user.id)  # Assuming your service handles skip/limit or you add it here
...
async def read_todo_by_id_api(  # Renamed
    ...
    db_todo = await get_todo_by_id(db, todo_id=todo_id)  # Fetch by ID using your service
...
async def update_existing_todo_api(  # Renamed
    ...
    updated_todo = await update_todo_service(
        ...
        description=update_data.get("description")  # Get description if present, else None
    )
    return updated_todo


@router.delete("/{todo_id}", status_code=status.HTTP_204_NO_CONTENT, summary="Delete Specific Todo")
